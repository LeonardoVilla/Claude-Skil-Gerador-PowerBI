...
- **Visuais** de cada página — tipo (card, donut, barras, slicer), campos e tabelas
...
|---|---|---|
| `MashupValidationError` | SecurityBindings não foi zerado | Gravar `b''` em SecurityBindings |
| "arquivo corrompido" | DataModel recomprimido (DEFLATED) | Preservar compress_type=0 para DataModel |
| `MashupValidationError` persistente | Template tem RemoteArtifacts | Trocar para template com DataModel local |
